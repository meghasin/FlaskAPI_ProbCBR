...
def get_model(args):
    ...
    return ProbCBR(args, args.train_map, args.full_map, eval_map, args.entity_vocab, args.rev_entity_vocab, args.rel_vocab,
                             args.rev_rel_vocab, args.eval_vocab, args.eval_rev_vocab, args.all_paths, args.rel_ent_map, args.per_relation_config, args.ans_num)
...
########### Load all paths ###########
file_prefix = "paths_{}_path_len_{}_".format(args.num_paths_around_entities, args.max_path_len)                                                                                            
args.all_paths = combine_path_splits(subgraph_dir, file_prefix=file_prefix)
all_kg_map = load_data_all_triples(train_file, os.path.join(data_dir, 'dev.txt'),
                                        os.path.join(data_dir, 'test.txt'))
args.all_kg_map = all_kg_map

args.train_map = load_data(kg_file)
args.full_map = load_data(kg_file)
#logger.info("Loading dev map")
dev_map = load_data(dev_file)
#logger.info("Loading test map")
#test_map = load_data(test_file)
#args.eval_map = dev_map
entity_vocab, rev_entity_vocab, rel_vocab, rev_rel_vocab, eval_vocab, eval_rev_vocab = load_vocab(data_dir)
args.entity_vocab = entity_vocab
args.rel_ent_map = get_entities_group_by_relation(train_file)
args.rev_entity_vocab = rev_entity_vocab
args.rel_vocab = rel_vocab
args.rev_rel_vocab = rev_rel_vocab
args.eval_vocab = eval_vocab
args.eval_rev_vocab = eval_rev_vocab
if args.per_relation_config_file is not None and os.path.exists(args.per_relation_config_file):
    args.per_relation_config = json.load(open(args.per_relation_config_file))
#logger.info("=========Per Relation Config:============")
    #     #logger.info(json.dumps(per_relation_config, indent=1, sort_keys=True))
else:
    args.per_relation_config = None
...
if __name__ == "__main__":
    flask_app.run(host=os.getenv('IP', '0.0.0.0'), port=int(os.getenv('PORT', 4440)),debug=True)
